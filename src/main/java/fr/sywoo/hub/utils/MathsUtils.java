...
public class MathsUtils {
    ...
    public Vector projectTo(Location from, Location to){
    	return to.toVector().subtract(from.toVector());
    }
    public Vector bumpEntity(Entity entity, Location from, double power){
    	Vector bump = entity.getLocation().toVector().subtract(from.toVector().normalize());
    	bump.multiply(power);
    	return bump;
    }
    public Vector pullEntity(Entity entity, Location to, double power){
