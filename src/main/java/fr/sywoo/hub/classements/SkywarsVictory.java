...
public class SkywarsVictory extends Display{

	public SkywarsVictory() {
		super("§a§lVictoires", new Location(-117, 68, 35).getAsLocation(), 10);
		display();
	}
	...
	public void display() {
		...
		for(AccountData data : LionSpigot.get().getAccountManager().getAllPlayers()) {
			...
			classement.put(data.getUUID(), (int) stats.getVictory());
		}
		...
		classement.entrySet().stream().sorted(Map.Entry.comparingByValue(Collections.reverseOrder())).limit(10).forEach(action -> {
			...
			this.spawnArmorStand("§f" + displayName + " : §c" + action.getValue() + " §bVictoires");
		});
